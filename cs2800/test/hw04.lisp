...
x = (list 2 3 4)
y = (list 4 5 6)
(reverse (app x y)) = (list 6 5 4 4 3 2)
(app (reverse x) (reverse y) = (list 4 3 2 6 5 4)
explanation: the order of the arguments to app matters, 
which means that the reverse of two appended lists only equates to the reverses of the two smaller lists
...
(PXVarp t) == t
(PXVarp nil) == t
In other words, per our data definition of PXVar,
boolean values are a subset of all possible PXVar values.
Looking back at our definition of propositional expressions, we know they are 
one of:
   boolean
   PXVar
   list...
   list...
Therefore, it is not possible to have a well-formed propositional expression 
that is neither a PXVar nor a list. The only other option is boolean, 
which we just proved is a kind of PXVar.
This means we cannot disprove the conjecture because we cannot create a propositional expression to counter it.
Therefore, the conjecutre is valid. (But it sucks, and so does our definition of PXVar.)
